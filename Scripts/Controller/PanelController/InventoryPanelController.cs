...
public class InventoryPanelController : MonoBehaviour
{
    ...
    public void UseSelectItem(PlayerStatus _player)
    {
        ...
        SetActiveItemInfo(false);
        if (inventorySlots[InventoryManager.Instance.IndexOfItem(selectItem)].CurItem.count > 0)
            inventorySlots[InventoryManager.Instance.IndexOfItem(selectItem)].UpdateItemCount();
        else
        {
