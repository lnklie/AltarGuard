...
public class PathFindController : MonoBehaviour
{
    ...
    public void PathFinding()
    {
        ...
        bottomLeft = Vector2Int.RoundToInt(status.gameObject.transform.position - new Vector3(100f, 100f));
        topRight = Vector2Int.RoundToInt(status.gameObject.transform.position + new Vector3(100f, 100f));
        ...
        for (int i = 0; i < sizeX; i++)
        {
            for (int j = 0; j < sizeY; j++)
            {
                ...
                foreach (Collider2D col in Physics2D.OverlapCircleAll(new Vector2(i + bottomLeft.x, j + bottomLeft.y), 0.4f))
                    if (col.gameObject.layer == status.gameObject.layer) isWall = true;
